  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-2 font-nunito">
      ...
      <div className="flex flex-col md:flex-row items-center gap-4 md:gap-6 mb-4">
        <h1 className="text-xl md:text-3xl font-bold text-gray-900 whitespace-nowrap">{category}</h1>
              ...
              <Link href={`${basePath}/${product.id}`} className="block">
                <div className="relative w-full pt-[100%] bg-[#f8f8f8] group">
                  {product.images ? (
                    <>
                      <Image
                        src={product.images[currentImageIndex[product.id] || 0] || '/images/placeholder.png'}
                        alt={product.name}
                        fill
